...
class PriceRepositoryH2AdapterTest {
    ...
    @Test
    public void findByBrandIdAndProductIdAndDateReturnsListOfPricesWhenPricesExist() {
        var brandId = 1L;
        var productId = 1L;
        var dateTime = LocalDateTime.now();
        var priceEntities = Collections.singletonList(new com.aafa.test.inditex.infrastructure.database.entities.PriceEntity());
        var priceMOs = Collections.singletonList(new com.aafa.test.inditex.domain.model.PriceMO());
        when(priceJPARepository.findPriceByBrandProductAndDateTime(brandId, productId, dateTime)).thenReturn(priceEntities);
        when(priceEntityMapper.toPriceList(priceEntities)).thenReturn(priceMOs);

        var actual = priceRepositoryH2Adapter.findByBrandIdAndProductIdAndDate(brandId, productId, dateTime);
