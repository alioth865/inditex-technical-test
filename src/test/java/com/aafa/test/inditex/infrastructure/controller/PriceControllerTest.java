...
public class PriceControllerTest {
    ...
    @Test
    public void getPriceByBrandAndProductAndDateReturnsPriceWhenPriceExists() {
        Long brandId = 1L;
        Long productId = 1L;
        ...
        when(getPriceUseCase.execute(productId, brandId, LocalDateTime.parse(dateTime, PriceController.DATE_TIME_FORMATTER)))
                .thenReturn(expectedPriceMO);
        when(priceResponseDtoMapper.toDto(expectedPriceMO)).thenReturn(expectedPriceResponse);

        ResponseEntity<PriceResponse> response = priceController.getPriceByBrandAndProductAndDate(brandId, productId, dateTime);

        assertEquals(ResponseEntity.ok(expectedPriceResponse), response);
    }

    @Test
    public void getPriceByBrandAndProductAndDateReturnsNullWhenNoPriceExists() {
        Long brandId = 1L;
        Long productId = 1L;
        String dateTime = "2022-12-31-23.59.59";
        when(getPriceUseCase.execute(productId, brandId, LocalDateTime.parse(dateTime, PriceController.DATE_TIME_FORMATTER)))
